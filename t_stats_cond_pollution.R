...
# for a selected list of numeric columns of dt, calculate difference between 
# chronic supercontrols - chronic max constrols 
compare_chrn_vs_chrn_max <- function(tb, lbl, logar){
  col1 <- tb[, 1]
  row_names <- tb[, get(colnames(tb)[1])]
  cls <- colnames(tb)[grep(lbl, colnames(tb))]
  cls_full <- c(colnames(tb)[1], cls)
  cond <- grep('chronic', row_names)
  cond_max <- grep('chronic_max', row_names)
  cond <- which(!grepl('chronic_max', row_names) & grepl('chronic', row_names)  )
  if(logar){
    dtb <- -log(as.matrix(tb[ cond, ..cls])) + log(as.matrix(tb[cond_max, ..cls]))
  } else {
    dtb <- as.matrix(tb[ cond, ..cls]) - as.matrix(tb[cond_max, ..cls])
  }
  dtb <- as.data.table(dtb)
  if(logar){names(dtb) <- paste('_log_', names(dtb), sep='')}
  dtb <- bind_cols(col1[cond, ], dtb)
  return(dtb)
}

# add as last two columns the optimal value and the argument of optimal value
# for a list of columns cols of a data table dt
# choose how to label the new columns by choosing label_val and label_arg
add_extremes <- function(dt, cols, foo, label_val, label_arg){
  dt <- mutate(dt, V = do.call(foo, dt[ ,..cols] ))
  args <- vector(mode="integer", length=nrow(dt))
  for( r in c(1:nrow(dt)) ){
    args[r] <- which(dt[r,..cols] == dt[r, V])
  }
  dt <- mutate(dt, W = cols[args])
  names(dt)[c( (ncol(dt) - 1) , ncol(dt) )] <- c(label_val, label_arg)
  return(dt)
}

add_extremes_abs <- function(dt, cols, foo, label_val, label_arg){
    #dt_cols <- abs(dt[ ,..cols])
    dt <- mutate(dt, V = do.call(foo, abs(dt[ ,..cols])))
    args <- vector(mode="integer", length=nrow(dt))
    for( r in c(1:nrow(dt)) ){
      args[r] <- which(abs(dt[r ,..cols]) == dt[r, V])
    }
    dt <- mutate(dt, W = cols[args])
    for( r in c(1:nrow(dt)) ){
      k <- dt[r, W]
...
# creates a table in whihc a selected list cols of columns of dt become rows
# and there is one column for te optimal values per cols of dt 
# and one columns for the arguments of the optimal values 
extr_per_columns <- function(dt, foo, cols, lbl_value, lbl_arg){
  cond <- dt[,get(names(dt)[1])]
  dt <- dt[,..cols]
  sm1 <- summarise_all(dt, foo, na.rm=TRUE)
  sm1 <- t(as.vector(sm1[1,]))
  #return(sm1)
  sm2 <- vector(mode="integer", length=length(cols))
  for(j in c(1:length(cols)) ){
    #t <- names(dt)[j]
    t <- cols[j]
    sm2[j] <- which(dt[, get(t)] == sm1[j] )
  }
  res <- data.table(cols, sm1, cond[sm2])
  names(res) <- c('pollutant_variable', lbl_value, lbl_arg)
  return(res)
}

extr_per_columns_abs <- function(dt, foo, cols, lbl_value, lbl_arg){
  cond <- dt[,get(names(dt)[1])]
  dt_abs <- abs(dt[,..cols])
  sm1 <- summarise_all(dt_abs, foo, na.rm=TRUE)
  sm1 <- t(as.vector(sm1[1,]))
  sm2 <- vector(mode="integer", length=length(cols))
  for(j in c(1:length(cols)) ){
    #t <- names(dt_abs)[j]
    t <- cols[j]
    sm2[j] <- which(dt_abs[, get(t)] == sm1[j] )
  }
  sm1 <- vector(mode="double", length=length(cols))
  for(r in c(1:ncol(dt_abs))){
    #t <- names(dt_abs)[r]
    t <- cols[j]
    sm1[r] <- dt[,..cols][sm2[r], get(t)]
  }
  res <- data.table(cols, sm1, cond[sm2])
  names(res) <- c('pollutant_variable', lbl_value, lbl_arg)
  return(res)
...
# acute pain: acute==0 control==3 and unknown== 1 (chronic pain) or -3 or -1 or NA 
...
write.table(P,'F:\\NIKO\\Genomics\\UK_Biobank\\Output_data_ph2\\pain_duration_answered.txt', 
            append = FALSE, sep = "\t", quote = FALSE, col.names=TRUE, row.names=FALSE)

View(fread('F:\\NIKO\\Genomics\\UK_Biobank\\Output_data_ph2\\pain_duration_answered.txt'))
...
# stats_table and stats_table_begn_eur are the final results of the t-stats test
# the subsequent tales with min/max columns (per row) 
# and min/max per column tables come from these two



##########################################################################
######## NEW STUFF, adding columns and rows with arguents and min-maxes #################
##########################################################################
#ready to go!

# we start with the
# stats_table and stats_table_begn_eur 
# which are the final results of the t-stats test
# the subsequent tales with min/max columns (per row) 
# and min/max per column tables come from these two


col_p  <- names(stats_table)[grep('p.value', names(stats_table))]
col_sd <- names(stats_table)[grep('diff_in_means_full_sd_units', names(stats_table))]

stats_table_p_min <- add_extremes(stats_table, col_p, pmin, 'p.value_min', 'pollutant_min_p')

stats_table_bgen_eur_p_min <- add_extremes(stats_table_bgen_eur, col_p, pmin, 'p.value_min', 'pollutant_min_p')

stats_table_3 <- add_extremes_abs(stats_table_p_min, col_sd, pmax, 'diff_of_means_sd_units_max', 'pollutant_diff_of_means_max')

stats_table_bgen_eur_3 <- add_extremes_abs(stats_table_bgen_eur_p_min, col_sd, pmax, 'diff_of_means_sd_units_max', 'pollutant_diff_of_means_max')

# full tables + p_min | argp_min | sd_max | argsd_max 
write.table(stats_table_3,'F:\\NIKO\\Genomics\\UK_Biobank\\Sumstats\\t_tests_pollution_in_asthma_pain\\pollution_in_asthma_pain_t_stats.txt', 
            append = FALSE, sep = "\t", quote = FALSE, col.names=TRUE, row.names=FALSE)

write.table(stats_table_bgen_eur_3,'F:\\NIKO\\Genomics\\UK_Biobank\\Sumstats\\t_tests_pollution_in_asthma_pain\\pollution_in_asthma_pain_t_stats_eur.txt', 
            append = FALSE, sep = "\t", quote = FALSE, col.names=TRUE, row.names=FALSE)


# stats_table_3 and stats_table_bgen_eur_3 are the full extended tables with added
# columns for optimal p.values and sd_differences



#View(add_extremes_abs(stats_table, col_sd, max, 'diff_of_means_sd_units_max', 'pollutant_diff_of_means_max'))

#test <- (add_extremes_abs(stats_table, col_sd, max, 'diff_of_means_sd_units_max', 'pollutant_diff_of_means_max'))

lbl_value <- 'max_diff_means' 
lbl_arg <- 'condition_ with_max_diff_means'
stats_table_2 <- extr_per_columns_abs(stats_table_3, max, col_sd, lbl_value, lbl_arg)

stats_table_bgen_eur_2 <- extr_per_columns_abs(stats_table_bgen_eur, max, col_sd, lbl_value, lbl_arg)

# small table pollutants max sd along conditions and its argmax
write.table(stats_table_2,'F:\\NIKO\\Genomics\\UK_Biobank\\Sumstats\\t_tests_pollution_in_asthma_pain\\pollution_in_asthma_pain_max_diff_sd.txt', 
            append = FALSE, sep = "\t", quote = FALSE, col.names=TRUE, row.names=FALSE)

write.table(stats_table_bgen_eur_2,'F:\\NIKO\\Genomics\\UK_Biobank\\Sumstats\\t_tests_pollution_in_asthma_pain\\pollution_in_asthma_pain_eur_max_diff_sd.txt', 
            append = FALSE, sep = "\t", quote = FALSE, col.names=TRUE, row.names=FALSE)

lbl_value <- 'min_p' 
lbl_arg <- 'condition_with_min_p'
write.table(extr_per_columns(stats_table_3, min, col_p, lbl_value, lbl_arg),'F:\\NIKO\\Genomics\\UK_Biobank\\Sumstats\\t_tests_pollution_in_asthma_pain\\pollution_in_asthma_pain_min_p.txt', 
            append = FALSE, sep = "\t", quote = FALSE, col.names=TRUE, row.names=FALSE)

write.table(extr_per_columns(stats_table_bgen_eur_3, min, col_p, lbl_value, lbl_arg),'F:\\NIKO\\Genomics\\UK_Biobank\\Sumstats\\t_tests_pollution_in_asthma_pain\\pollution_in_asthma_pain_min_p_eur.txt', 
            append = FALSE, sep = "\t", quote = FALSE, col.names=TRUE, row.names=FALSE)


# stats_table_2 and _bgen_eur_2 are the small transposed tables per column



# exporting only the parts of tables _3 with p.value and sd_difference
col_p3  <- names(stats_table)[grep('p.value', names(stats_table))]
#s <- c(names(stats_table_3)[1] , col_p)

s <- c(names(stats_table_3)[1] , col_p, names(stats_table_3)[c( (ncol(stats_table_3)-3) :  (ncol(stats_table_3) - 0)  )] )
View(stats_table_3[, ..s])
View(stats_table_bgen_eur_3[, ..s])

write.table(stats_table_3[, ..s], 'F:\\NIKO\\Genomics\\UK_Biobank\\Sumstats\\t_tests_pollution_in_asthma_pain\\pollution_p_values_only.txt', 
            append = FALSE, sep = "\t", quote = FALSE, col.names=TRUE, row.names=FALSE)

write.table(stats_table_bgen_eur_3[, ..s], 'F:\\NIKO\\Genomics\\UK_Biobank\\Sumstats\\t_tests_pollution_in_asthma_pain\\pollution_bgen_eur_p_values_only.txt', 
            append = FALSE, sep = "\t", quote = FALSE, col.names=TRUE, row.names=FALSE)


s <- c(names(stats_table_3)[1] , col_sd, names(stats_table_3)[c( (ncol(stats_table_3)-3) :  (ncol(stats_table_3) - 0)  )] )
View(stats_table_3[, ..s])
View(stats_table_bgen_eur_3[, ..s])

write.table(stats_table_3[, ..s], 'F:\\NIKO\\Genomics\\UK_Biobank\\Sumstats\\t_tests_pollution_in_asthma_pain\\pollution_sd_diff_only.txt', 
            append = FALSE, sep = "\t", quote = FALSE, col.names=TRUE, row.names=FALSE)

write.table(stats_table_bgen_eur_3[, ..s], 'F:\\NIKO\\Genomics\\UK_Biobank\\Sumstats\\t_tests_pollution_in_asthma_pain\\pollution_bgen_eur_sd_diff_only.txt', 
            append = FALSE, sep = "\t", quote = FALSE, col.names=TRUE, row.names=FALSE)



####################################
# comapre the p.values and sd_diff of chroninc and chronic_max

compare_table_p <- compare_chrn_vs_chrn_max(stats_table, 'p.value', logar=TRUE)
compare_table_p_eur <- compare_chrn_vs_chrn_max(stats_table_bgen_eur, 'p.value', logar=TRUE)

compare_table_sd <- compare_chrn_vs_chrn_max(stats_table, 'diff_in_means_full_sd_units', logar=FALSE)
compare_table_sd_eur <- compare_chrn_vs_chrn_max(stats_table_bgen_eur, 'diff_in_means_full_sd_units', logar=FALSE)


write.table(compare_table_p, 'F:\\NIKO\\Genomics\\UK_Biobank\\Sumstats\\t_tests_pollution_in_asthma_pain\\pollution_compare_table_p.txt', 
            append = FALSE, sep = "\t", quote = FALSE, col.names=TRUE, row.names=FALSE)

write.table(compare_table_p_eur, 'F:\\NIKO\\Genomics\\UK_Biobank\\Sumstats\\t_tests_pollution_in_asthma_pain\\pollution_compare_table_p_eur.txt', 
            append = FALSE, sep = "\t", quote = FALSE, col.names=TRUE, row.names=FALSE)

write.table(compare_table_sd, 'F:\\NIKO\\Genomics\\UK_Biobank\\Sumstats\\t_tests_pollution_in_asthma_pain\\pollution_compare_table_sd.txt', 
            append = FALSE, sep = "\t", quote = FALSE, col.names=TRUE, row.names=FALSE)

write.table(compare_table_sd_eur, 'F:\\NIKO\\Genomics\\UK_Biobank\\Sumstats\\t_tests_pollution_in_asthma_pain\\pollution_compare_table_sd_eur.txt', 
            append = FALSE, sep = "\t", quote = FALSE, col.names=TRUE, row.names=FALSE)
...
###### adding minimal p_value (old stuff):
...
# log(p_values) 
col_p_sd <- c(col_p, col_sd)
s <- c(names(stats_table_3)[1] , col_p_sd, names(stats_table_3)[c( (ncol(stats_table_3)-3) :  (ncol(stats_table_3) - 0)  )] )

cnd <- stats_table_bgen_eur_3[, get(names(stats_table_bgen_eur_3)[1])]
cnd <- cnd[grep('chronic', cnd)]
cnd_max <- cnd[grep('_max', cnd)]
cnd <- cnd[!grepl('_max', cnd)]
stats_table_3[, ..s]
stats_table_bgen_eur_3[, ..s]

tb_chrn <- stats_table_3[med_condition %in% cnd, ..s]
tb_chrn_eur <- stats_table_bgen_eur_3[med_condition %in% cnd, ..s]

tb_chrn_max <- stats_table_3[med_condition %in% cnd_max, ..s]
tb_chrn_eur_max <- stats_table_bgen_eur_3[med_condition %in% cnd_max, ..s]

View(- log(as.matrix(tb_chrn)) + log(as.matrix(tb_chrn_max)))
...
View(fread('F:\\NIKO\\Genomics\\UK_Biobank\\Sumstats\\t_tests_pollution_in_asthma_pain\\pollution_in_asthma_pain_eur_t_stats.txt'))

View(fread('F:\\NIKO\\Genomics\\UK_Biobank\\Sumstats\\t_tests_pollution_in_asthma_pain\\pollution_in_asthma_pain_t_stats.txt'))

cls <- c(1, grep('p.value', names(stats_table_1)))

View(stats_table_1[ ,..cls])
write.table(stats_table_1[ ,..cls],'F:\\NIKO\\Genomics\\UK_Biobank\\Sumstats\\t_tests_pollution_in_asthma_pain\\pollution_in_asthma_pain_t_stats_p_vals.txt', 
            append = FALSE, sep = "\t", quote = FALSE, col.names=TRUE, row.names=FALSE)

cls <- c(1, grep('p.value', names(stats_table_bgen_eur_1)))

View(stats_table_bgen_eur_1[ ,..cls])
write.table(stats_table_bgen_eur_1[ ,..cls],'F:\\NIKO\\Genomics\\UK_Biobank\\Sumstats\\t_tests_pollution_in_asthma_pain\\pollution_in_asthma_pain_eur_t_stats_p_vals.txt', 
            append = FALSE, sep = "\t", quote = FALSE, col.names=TRUE, row.names=FALSE)
